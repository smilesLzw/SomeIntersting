...
class Connection(object):
    ...
    def reconnect(self):
        """
        Closes the existing database connecting and re-open it.
        """
        self.close()
    ...
    def execute(self, query, *parameters, **kwparameters):
        """Executes the given query, returning the lastrowid from query"""
        cursor = self._cursor()
        try:
            cursor.execute(query, kwparameters or parameters)
    ...
    def table_update(self, table_name, updates, field_where, value_where):
        ...
        for k, v in updates.items():
            ...
            values.append(v)
        upsets = ','.join(upsets)
        sql = 'UPDATE %s SET %s WHERE %s="%s"' % (table_name, upsets,
